...
describe('SPAM Expressions', () => {
  describe('parse', () => {
    ...
    describe('TokenTag', () => {
      ...
      it(`<| ID foo='bar'vroom='graarrr' |> throws`, () => {
        expect(() => parse(`<| ID foo='bar'vroom='graarrr' |>`)).toThrowError();
      });
      ...
      it(`<| ID { re: // } |>`, () => {
        expect(parse(`<| ID { re: // } |>`)).toEqual({
          type: 'TokenTag',
          tagType: 'ID',
          value: null,
          attrs: [],
          args: [{ type: 'Argument', key: 're', value: emptyRegex }],
        });
      });
      ...
      it(`<| ID { foo: 'bar', baz: 'quux' } |>`, () => {
        expect(parse(`<| ID { foo: 'bar', baz: 'quux' } |>`)).toEqual({
          ...
          attrs: [],
          args: [
            { type: 'Argument', key: 'foo', value: { type: 'String', value: 'bar' } },
            { type: 'Argument', key: 'baz', value: { type: 'String', value: 'quux' } },
          ],
        });
      });

      it(`<| ID { kw: <| KW |> } |>`, () => {
        expect(parse(`<| ID { kw: <| KW |> } |>`)).toEqual({
          type: 'TokenTag',
          tagType: 'ID',
          value: null,
          attrs: [],
