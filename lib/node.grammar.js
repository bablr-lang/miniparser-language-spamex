...
export default class SpamexNodeGrammar {
  constructor() {
    this.covers = buildCovers({
      [node]: ['Attribute', 'Argument', 'Identifier', 'Expression'],
  ...
  *TokenMatcher() {
    ...
    const attrs = sp && (yield i`eat(<Attributes>)`);
  ...
  *Attributes() {
    ...
    while (
      sp &&
      (yield i`match(/\w/)` || (p.atQuasi && (isArray(p.quasi) || p.quasi.type === 'Attribute')))
    ) {
      eatProductions(spam`<Attribute path='attributes'>`);
  ...
  *Attribute() {
    ...
    yield i`eatMatch(<| |>)`;
    yield i`eat(<String:String path='value'>)`;
  }
